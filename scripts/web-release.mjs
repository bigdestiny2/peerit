...
const CANARY_SEQ23_FETCH_BINDING_DECISION_FILE =
  'deploy/canary-decision-peerit-seq23-fetch-binding-recovery-fix-20260801.json'
const CANARY_SEQ23_FETCH_BINDING_DECISION_SHA256 =
  '2ded91c43bfaccfe5d227249d0708a90eb29f04a2c3c10e0755de85a1c612f08'
const CANARY_SEQ24_DESCRIPTOR_DRIVEN_DECISION_FILE =
  'deploy/canary-decision-peerit-seq24-descriptor-driven-admission-20260802.json'
const CANARY_SEQ24_DESCRIPTOR_DRIVEN_DECISION_SHA256 =
  'd29b9ec3eb1e7f8c20df1e4a4cccbfab99dfae849b574b0118e34d5fee5c6c25'
const CANARY_SEQ25_FIRST_VISIT_RECOVERY_DECISION_FILE =
  'deploy/canary-decision-peerit-seq25-first-visit-recovery-20260805.json'
const CANARY_SEQ25_FIRST_VISIT_RECOVERY_DECISION_SHA256 =
  'd5e8f263045f7e0cbcdacbfe8d873959a1432561fff9f5f0ccb84c73128e9093'
const CANARY_SEQ26_FIRST_VISIT_UX_HONESTY_DECISION_FILE =
  'deploy/canary-decision-peerit-seq26-first-visit-ux-honesty-20260806.json'
const CANARY_SEQ26_FIRST_VISIT_UX_HONESTY_DECISION_SHA256 =
  '6fea93c3dde24637344da71d65c076cd776564bd6dd261935588d6fce4b063df'
const CANARY_SEQ27_RESEED_DISTRIBUTION_DECISION_FILE =
...
function verifyCanaryOwnerDecision (release) {
  ...
  if (release.releaseSequence === 27) {
    const decision = readCanaryOwnerDecision(
      ...
      CANARY_SEQ27_RESEED_DISTRIBUTION_DECISION_SHA256)
    ...
    const superseded = rootCause.superseded_cells_documented || {}
    ...
    if (decision.schema_version !== 5 ||
        decision.decision_id !== 'peerit-seq27-reseed-distribution-20260806' ||
        decision.status !== 'DECIDED' ||
        !String(decision.decision || '').startsWith('ACCEPT Peerit release sequence 27 as the LIVE bounded-public-test launch successor') ||
        !followups.includes('GA product gate remains honestly blocked')) {
      throw new Error('sequence-27 reseed distribution decision is not the recorded owner ACCEPT')
    }
    if (authority.baseline_release_sequence !== 26 ||
        !Array.isArray(authority.cited_prior_decisions) ||
        !authority.cited_prior_decisions.some(row => row.file === CANARY_SEQ26_FIRST_VISIT_UX_HONESTY_DECISION_FILE && row.sha256 === CANARY_SEQ26_FIRST_VISIT_UX_HONESTY_DECISION_SHA256)) {
      throw new Error('sequence-27 decision does not cite the seq-26 ux-honesty decision it extends')
    }
    if (!Array.isArray(rootCause.correction_applied) ||
        rootCause.correction_applied.length < 3 ||
        !rootCause.correction_applied.some(row => String(row).includes('Community swap')) ||
        !rootCause.correction_applied.some(row => String(row).includes('re-minted')) ||
        !rootCause.correction_applied.some(row => String(row).includes('byte-exact readback')) ||
        superseded.count !== 13 ||
        !String(superseded.no_protocol_purge || '').includes('dropPrivateKey') ||
        !String(rootCause.explainer_page || '').includes('how-it-works.html') ||
        !String(rootCause.no_weakening || '').startsWith('no verification')) {
      throw new Error('sequence-27 decision does not bind the exact reseed distribution + superseded + explainer scope')
    }
    if (activation.functional_release_sequence !== 27 ||
        ...
        activation.limited_cell_get_authority_release_sequence !== 27 ||
        ...
        activation.browser_authored_records !== 'LOCAL_ONLY' ||
      ...
      throw new Error('sequence-27 decision does not bind the exact live launch successor scope')
    }
    if (!Array.isArray(extension.files) ||
        !extension.files.includes('how-it-works.html') ||
        !extension.files.includes('publish.mjs') ||
        !extension.files.includes('js/substrate/peerit-product-ui.js') ||
      ...
      throw new Error('sequence-27 decision does not record the ceremony extension surface')
    }
    if (bootstrap.path !== 'deploy/peerit-seed-bootstrap-v1-seq27.json' ||
        bootstrap.sha256 !== 'cc7ce9a92967f6fdbec5038a612f996810935394b8c91db31c9c3d53ab25b77b' ||
        bootstrap.embedded_release_sequence !== 27 ||
      ...
      throw new Error('sequence-27 decision does not bind the exact redistributed launch seed bootstrap')
    }
    if (csp.policy_file !== 'deploy/render-security-headers.json' ||
      ...
      throw new Error('sequence-27 decision does not bind the exact unchanged policy')
    }
    if (exactUrl.utf8 !== 'https://evidence.example:443/admission.cenc' ||
      ...
      throw new Error('sequence-27 decision does not bind the exact no-fetch parameterUrl contract')
    }
    const gate27 = runtimeGate.sequence_27 || {}
    if (runtimeGate.script !== 'scripts/browser-peerit-production-runtime-gate.mjs' ||
        runtimeGate.functional_mode !== 'live-two-relay' ||
        runtimeGate.functional_release_sequence !== 27 ||
        ...
        gate27.expected_network_gets !== 40 || gate27.expected_fallback_count !== 1 ||
        gate27.expected_record_count !== 39 || gate27.expected_cell_get_requests !== 40 ||
        gate27.expected_successful_cell_gets !== 39 ||
        gate27.expected_network_puts !== 0 || gate27.expected_parameter_url_requests !== 0 ||
        gate27.expected_sizeclass_1_responses !== 36 ||
        gate27.expected_sizeclass_2_responses !== 3 ||
        gate27['dal-1']?.origin !== 'https://relay-dal.p2phiverelay.xyz' ||
        gate27['dal-1']?.injected_cell_get_failures !== 1 ||
        gate27['dal-1']?.successful_cell_gets !== 38 ||
        gate27['dal-1']?.verified_readback_evidence_count !== 38 ||
        gate27['syd-1']?.origin !== 'https://relay-syd.p2phiverelay.xyz' ||
        gate27['syd-1']?.injected_cell_get_failures !== 0 ||
        gate27['syd-1']?.successful_cell_gets !== 1 ||
        gate27['syd-1']?.verified_readback_evidence_count !== 1) {
      throw new Error('sequence-27 decision does not bind the named-relay production runtime gate')
    }
    if (relays.admission_semantics == null ||
        !String(relays.admission_semantics).startsWith('descriptor-driven') ||
        relays['dal-1']?.minimum_descriptor_sequence !== 21 ||
        relays['dal-1']?.descriptor_head_sha256 !== '3986870f1b56ec0ac601b9f0e0b939af9980d6568febb03aae5907d0ba721dc1' ||
        relays['syd-1']?.minimum_descriptor_sequence !== 24 ||
        relays['syd-1']?.descriptor_head_sha256 !== 'd4cae7212ff6da966c811eed4bab4f9195aaf822cd4a131dc6fa3079b273b881' ||
        relays['dal-1']?.admission_protocol_sha256 != null ||
        relays['syd-1']?.admission_protocol_sha256 != null) {
      throw new Error('sequence-27 decision does not bind the descriptor-driven admission authority (descriptor pins present, admission hash deliberately absent)')
    }
    addCheck('canary:owner-decision', 'pass', `Owner reseed distribution decision verified byte-exact (sha256 ${CANARY_SEQ27_RESEED_DISTRIBUTION_DECISION_SHA256.slice(0, 12)}...): ACCEPT seq-27 as the LIVE bounded-public-test launch successor — owner-approved seed re-distribution (community swap applied, 7 content boards / 4 bounty boards, 7 posts re-minted to correct boards, 13 superseded cells documented + lease-GC) + standalone product explainer page (how-it-works.html); zero verification/transport change; limited Cell-GET exposed, 39-record signed recovery enabled; unchanged CSP, zero PUTs, all-five excluded, GA gate still blocked with 22 blockers DISCLOSED-OPEN.`, {
      file: CANARY_SEQ27_RESEED_DISTRIBUTION_DECISION_FILE,
      sha256: CANARY_SEQ27_RESEED_DISTRIBUTION_DECISION_SHA256,
      decidedAt: decision.decided_at,
      functionalReleaseSequence: 27,
      launchRecords: launch.record_count,
      launchCellsPerRelay: launch.cell_count_per_relay
    })
    return
  }
  if (release.releaseSequence === 26) {
    const decision = readCanaryOwnerDecision(
      CANARY_SEQ26_FIRST_VISIT_UX_HONESTY_DECISION_FILE,
      CANARY_SEQ26_FIRST_VISIT_UX_HONESTY_DECISION_SHA256)
    const activation = decision.activation || {}
    const authority = decision.authority || {}
    const launch = activation.launch_seed || {}
    const rootCause = decision.root_cause || {}
    ...
    const followups = Array.isArray(decision.followups) ? decision.followups.join('\n') : ''
    if (decision.schema_version !== 5 ||
        decision.decision_id !== 'peerit-seq26-first-visit-ux-honesty-20260806' ||
        decision.status !== 'DECIDED' ||
        !String(decision.decision || '').startsWith('ACCEPT Peerit release sequence 26 as the LIVE bounded-public-test launch successor') ||
        !followups.includes('GA product gate remains honestly blocked')) {
      throw new Error('sequence-26 first-visit ux-honesty decision is not the recorded owner ACCEPT')
    }
    if (authority.baseline_release_sequence !== 25 ||
        !Array.isArray(authority.cited_prior_decisions) ||
        !authority.cited_prior_decisions.some(row => row.file === CANARY_SEQ25_FIRST_VISIT_RECOVERY_DECISION_FILE && row.sha256 === CANARY_SEQ25_FIRST_VISIT_RECOVERY_DECISION_SHA256)) {
      throw new Error('sequence-26 decision does not cite the seq-25 first-visit recovery decision it extends')
    }
    if (!Array.isArray(rootCause.ux_changes_applied) ||
        rootCause.ux_changes_applied.length < 3 ||
        !rootCause.ux_changes_applied.some(row => String(row).includes('boot-sub')) ||
        !rootCause.ux_changes_applied.some(row => String(row).includes('read more')) ||
        !rootCause.ux_changes_applied.some(row => String(row).includes('local') && String(row).includes('milestone')) ||
        !String(rootCause.local_only_honesty || '').includes('LOCAL_ONLY') ||
        !String(rootCause.no_verification_or_transport_change || '').startsWith('No verification')) {
      throw new Error('sequence-26 decision does not bind the exact UX-honesty copy scope')
    }
    if (activation.functional_release_sequence !== 26 ||
        activation.rollback_release_sequence !== null ||
        activation.rollback_posture !== 'SUPERSEDED_FOR_THIS_SLOT_BY_OWNER_DECISION_2026-08-06' ||
        activation.limited_cell_get_authority_release_sequence !== 26 ||
        activation.limited_cell_get_runtime_authority_exposed !== true ||
        activation.seed_recovery_enabled !== true ||
        activation.claim_boundary !== 'LIVE_PUBLIC_TEST_ONLY' ||
        activation.browser_authored_records !== 'LOCAL_ONLY' ||
        ...
        activation.all_five_successor !== 'EXCLUDED' ||
        activation.ga_product_gate !== 'BLOCKED — 22 blockers DISCLOSED-OPEN, none cleared by this scope') {
      throw new Error('sequence-26 decision does not bind the exact live launch successor scope')
    }
    if (!Array.isArray(extension.files) ||
        !extension.files.includes('index.html') ||
        !extension.files.includes('scripts/build-single-file.mjs') ||
        !extension.files.includes('js/substrate/peerit-product-ui.js') ||
        !extension.files.includes('js/substrate/limited-cell-get-profile.mjs') ||
        !extension.files.includes('peerit-limited-cell-get-profile-v1.json') ||
        !extension.files.includes('scripts/production-pin-history-ceremony.mjs')) {
      throw new Error('sequence-26 decision does not record the ceremony extension surface')
    }
    if (bootstrap.path !== 'deploy/peerit-seed-bootstrap-v1-seq26.json' ||
        bootstrap.sha256 !== '057ab1e1a888be8cf3d70ebef034bbd13488517652508e9a7700e5322637e64c' ||
        bootstrap.embedded_release_sequence !== 26 ||
        bootstrap.bootstrap_sequence !== 0 ||
        bootstrap.previous_bootstrap_hash !== null ||
        bootstrap.discovery_authority !== '691d524a1c2ac38de86ed592fbae6f9a906770b96fe704d3c63397a23171f6ec' ||
        bootstrap.records !== 39) {
      throw new Error('sequence-26 decision does not bind the exact launch seed bootstrap')
    }
    if (csp.policy_file !== 'deploy/render-security-headers.json' ||
        csp.policy_file_sha256 !== 'e672153d1c396e617491fce64ed5472635314e20c45864e959b48e5f1b52b312' ||
        csp.script_src !== "'self'" || csp.unsafe_eval !== 'FORBIDDEN' ||
        csp.wasm_unsafe_eval !== 'FORBIDDEN' || csp.expansion !== 'FORBIDDEN' ||
        csp.parameter_url_origin_addition !== 'FORBIDDEN') {
      throw new Error('sequence-26 decision does not bind the exact unchanged policy')
    }
    if (exactUrl.utf8 !== 'https://evidence.example:443/admission.cenc' ||
        exactUrl.utf8_hex !==
          '68747470733a2f2f65766964656e63652e6578616d706c653a3434332f61646d697373696f6e2e63656e63' ||
        ...
        exactUrl.csp_change !== 'FORBIDDEN' ||
        exactUrl.comparison !== 'EXACT_SIGNED_UTF8_BYTES') {
      throw new Error('sequence-26 decision does not bind the exact no-fetch parameterUrl contract')
    }
    const gate26 = runtimeGate.sequence_26 || {}
    if (runtimeGate.script !== 'scripts/browser-peerit-production-runtime-gate.mjs' ||
        runtimeGate.functional_mode !== 'live-two-relay' ||
        runtimeGate.functional_release_sequence !== 26 ||
        runtimeGate.full_authority_loader !== 'loadPeeritBrowserRuntimeAuthorityV1' ||
        runtimeGate.production_pin_history_loader !== 'loadPeeritProductionPinHistoryTerminalV1' ||
        runtimeGate.authority_active_before_relay_io !== true ||
        gate26.expected_network_gets !== 40 || gate26.expected_fallback_count !== 1 ||
        gate26.expected_record_count !== 39 || gate26.expected_cell_get_requests !== 40 ||
        gate26.expected_successful_cell_gets !== 39 ||
        gate26.expected_network_puts !== 0 || gate26.expected_parameter_url_requests !== 0 ||
        gate26.expected_sizeclass_1_responses !== 36 ||
        gate26.expected_sizeclass_2_responses !== 3 ||
        gate26['dal-1']?.origin !== 'https://relay-dal.p2phiverelay.xyz' ||
        gate26['dal-1']?.injected_cell_get_failures !== 1 ||
        gate26['dal-1']?.successful_cell_gets !== 38 ||
        gate26['dal-1']?.verified_readback_evidence_count !== 38 ||
        gate26['syd-1']?.origin !== 'https://relay-syd.p2phiverelay.xyz' ||
        gate26['syd-1']?.injected_cell_get_failures !== 0 ||
        gate26['syd-1']?.successful_cell_gets !== 1 ||
        gate26['syd-1']?.verified_readback_evidence_count !== 1) {
      throw new Error('sequence-26 decision does not bind the named-relay production runtime gate')
    }
    if (relays.admission_semantics == null ||
        !String(relays.admission_semantics).startsWith('descriptor-driven') ||
        relays['dal-1']?.minimum_descriptor_sequence !== 20 ||
        relays['dal-1']?.descriptor_head_sha256 !== '56edc91e12e141be4387fa3f96aa9c74595e6c93db306e33b784ac43e008cf46' ||
        relays['syd-1']?.minimum_descriptor_sequence !== 23 ||
        relays['syd-1']?.descriptor_head_sha256 !== 'e2a97e77badddac29234a48c0fff185df1646baeb7fcc291dfc495326361da78' ||
        relays['dal-1']?.admission_protocol_sha256 != null ||
        relays['syd-1']?.admission_protocol_sha256 != null) {
      throw new Error('sequence-26 decision does not bind the descriptor-driven admission authority (descriptor pins present, admission hash deliberately absent)')
    }
    addCheck('canary:owner-decision', 'pass', `Owner first-visit ux-honesty decision verified byte-exact (sha256 ${CANARY_SEQ26_FIRST_VISIT_UX_HONESTY_DECISION_SHA256.slice(0, 12)}...): ACCEPT seq-26 as the LIVE bounded-public-test launch successor — first-visit boot-time indication + excerpt read-more affordance + composer local-only honesty; browser-authored records remain LOCAL_ONLY (no public CELL.PUT, cross-device sync roadmap T2); zero verification/transport change; limited Cell-GET exposed, 39-record signed recovery enabled; unchanged CSP, zero PUTs, all-five excluded, GA gate still blocked with 22 blockers DISCLOSED-OPEN.`, {
      file: CANARY_SEQ26_FIRST_VISIT_UX_HONESTY_DECISION_FILE,
      sha256: CANARY_SEQ26_FIRST_VISIT_UX_HONESTY_DECISION_SHA256,
      decidedAt: decision.decided_at,
      functionalReleaseSequence: 26,
      launchRecords: launch.record_count,
      launchCellsPerRelay: launch.cell_count_per_relay
    })
    return
  }
  if (release.releaseSequence === 25) {
    const decision = readCanaryOwnerDecision(
      CANARY_SEQ25_FIRST_VISIT_RECOVERY_DECISION_FILE,
      CANARY_SEQ25_FIRST_VISIT_RECOVERY_DECISION_SHA256)
    const activation = decision.activation || {}
    const authority = decision.authority || {}
    const launch = activation.launch_seed || {}
    const rootCause = decision.root_cause || {}
    ...
    const followups = Array.isArray(decision.followups) ? decision.followups.join('\n') : ''
    if (decision.schema_version !== 5 ||
        decision.decision_id !== 'peerit-seq25-first-visit-recovery-20260805' ||
        decision.status !== 'DECIDED' ||
        !String(decision.decision || '').startsWith('ACCEPT Peerit release sequence 25 as the LIVE bounded-public-test launch successor') ||
        !followups.includes('GA product gate remains honestly blocked')) {
      throw new Error('sequence-25 first-visit recovery decision is not the recorded owner ACCEPT')
    }
    if (authority.baseline_release_sequence !== 24 ||
        !Array.isArray(authority.cited_prior_decisions) ||
        !authority.cited_prior_decisions.some(row => row.file === CANARY_SEQ24_DESCRIPTOR_DRIVEN_DECISION_FILE && row.sha256 === CANARY_SEQ24_DESCRIPTOR_DRIVEN_DECISION_SHA256)) {
      throw new Error('sequence-25 decision does not cite the seq-24 descriptor-driven decision it extends')
    }
    if (!Array.isArray(rootCause.liveness_fix_applied) ||
        rootCause.liveness_fix_applied.length < 2 ||
        !rootCause.liveness_fix_applied.some(row => String(row).includes('fetchDescriptorWithTransientRetry')) ||
        !rootCause.liveness_fix_applied.some(row => String(row).includes('cold-reader')) ||
        !String(rootCause.no_weakening || '').includes('descriptor signature')) {
      throw new Error('sequence-25 decision does not bind the exact transient-retry liveness fix')
    }
    if (activation.functional_release_sequence !== 25 ||
        activation.rollback_release_sequence !== null ||
        activation.rollback_posture !== 'SUPERSEDED_FOR_THIS_SLOT_BY_OWNER_DECISION_2026-08-05' ||
        activation.limited_cell_get_authority_release_sequence !== 25 ||
        activation.limited_cell_get_runtime_authority_exposed !== true ||
        activation.seed_recovery_enabled !== true ||
        activation.claim_boundary !== 'LIVE_PUBLIC_TEST_ONLY' ||
        JSON.stringify(activation.relays) !== JSON.stringify(['dal-1', 'syd-1']) ||
        ...
        activation.all_five_successor !== 'EXCLUDED' ||
        activation.ga_product_gate !== 'BLOCKED — 22 blockers DISCLOSED-OPEN, none cleared by this scope') {
      throw new Error('sequence-25 decision does not bind the exact live launch successor scope')
    }
    if (!Array.isArray(extension.files) ||
        !extension.files.includes('js/substrate/relay-consumer.js') ||
        !extension.files.includes('js/substrate/cold-reader.mjs') ||
        !extension.files.includes('js/substrate/limited-cell-get-profile.mjs') ||
        !extension.files.includes('peerit-limited-cell-get-profile-v1.json') ||
        !extension.files.includes('scripts/production-pin-history-ceremony.mjs')) {
      throw new Error('sequence-25 decision does not record the ceremony extension surface')
    }
    if (bootstrap.path !== 'deploy/peerit-seed-bootstrap-v1-seq25.json' ||
        bootstrap.sha256 !== 'e1e0879adf0fa3fd5b78f67ecd8a88ff17f28adcc1eab4c487f9872afa3b98b3' ||
        bootstrap.embedded_release_sequence !== 25 ||
        bootstrap.bootstrap_sequence !== 0 ||
        bootstrap.previous_bootstrap_hash !== null ||
        bootstrap.discovery_authority !== '691d524a1c2ac38de86ed592fbae6f9a906770b96fe704d3c63397a23171f6ec' ||
        bootstrap.records !== 39) {
      throw new Error('sequence-25 decision does not bind the exact launch seed bootstrap')
    }
    if (csp.policy_file !== 'deploy/render-security-headers.json' ||
        ...
        csp.wasm_unsafe_eval !== 'FORBIDDEN' || csp.expansion !== 'FORBIDDEN' ||
        csp.parameter_url_origin_addition !== 'FORBIDDEN') {
      throw new Error('sequence-25 decision does not bind the exact unchanged policy')
    }
    if (exactUrl.utf8 !== 'https://evidence.example:443/admission.cenc' ||
        exactUrl.utf8_hex !==
          '68747470733a2f2f65766964656e63652e6578616d706c653a3434332f61646d697373696f6e2e63656e63' ||
        ...
        exactUrl.csp_change !== 'FORBIDDEN' ||
        exactUrl.comparison !== 'EXACT_SIGNED_UTF8_BYTES') {
      throw new Error('sequence-25 decision does not bind the exact no-fetch parameterUrl contract')
    }
    const gate25 = runtimeGate.sequence_25 || {}
    if (runtimeGate.script !== 'scripts/browser-peerit-production-runtime-gate.mjs' ||
        runtimeGate.functional_mode !== 'live-two-relay' ||
        runtimeGate.functional_release_sequence !== 25 ||
        runtimeGate.full_authority_loader !== 'loadPeeritBrowserRuntimeAuthorityV1' ||
        runtimeGate.production_pin_history_loader !== 'loadPeeritProductionPinHistoryTerminalV1' ||
        runtimeGate.authority_active_before_relay_io !== true ||
        gate25.expected_network_gets !== 40 || gate25.expected_fallback_count !== 1 ||
        gate25.expected_record_count !== 39 || gate25.expected_cell_get_requests !== 40 ||
        gate25.expected_successful_cell_gets !== 39 ||
        gate25.expected_network_puts !== 0 || gate25.expected_parameter_url_requests !== 0 ||
        gate25.expected_sizeclass_1_responses !== 36 ||
        gate25.expected_sizeclass_2_responses !== 3 ||
        gate25['dal-1']?.origin !== 'https://relay-dal.p2phiverelay.xyz' ||
        gate25['dal-1']?.injected_cell_get_failures !== 1 ||
        gate25['dal-1']?.successful_cell_gets !== 38 ||
        gate25['dal-1']?.verified_readback_evidence_count !== 38 ||
        gate25['syd-1']?.origin !== 'https://relay-syd.p2phiverelay.xyz' ||
        gate25['syd-1']?.injected_cell_get_failures !== 0 ||
        gate25['syd-1']?.successful_cell_gets !== 1 ||
        gate25['syd-1']?.verified_readback_evidence_count !== 1) {
      throw new Error('sequence-25 decision does not bind the named-relay production runtime gate')
    }
    if (relays.admission_semantics == null ||
        !String(relays.admission_semantics).startsWith('descriptor-driven') ||
        relays['dal-1']?.minimum_descriptor_sequence !== 19 ||
        relays['dal-1']?.descriptor_head_sha256 !== 'f13ea495ba3d50057f77e71ff40b020eb672bd98f51b42a53474ffacfc590f65' ||
        relays['syd-1']?.minimum_descriptor_sequence !== 22 ||
        relays['syd-1']?.descriptor_head_sha256 !== '5690f7faf41492291f8c93dd21dbfa1c466e56b7a818857b6f0ceb8d6f7acaec' ||
        relays['dal-1']?.admission_protocol_sha256 != null ||
        relays['syd-1']?.admission_protocol_sha256 != null) {
      throw new Error('sequence-25 decision does not bind the descriptor-driven admission authority (descriptor pins present, admission hash deliberately absent)')
    }
    addCheck('canary:owner-decision', 'pass', `Owner first-visit recovery decision verified byte-exact (sha256 ${CANARY_SEQ25_FIRST_VISIT_RECOVERY_DECISION_SHA256.slice(0, 12)}...): ACCEPT seq-25 as the LIVE bounded-public-test launch successor — cold descriptor walk + Cell-GET fallback retry ONLY transient transport failures (bounded, backoff), no verification relaxation, limited Cell-GET exposed, 39-record signed recovery enabled; unchanged CSP, zero PUTs, all-five excluded, GA gate still blocked with 22 blockers DISCLOSED-OPEN.`, {
      file: CANARY_SEQ25_FIRST_VISIT_RECOVERY_DECISION_FILE,
      sha256: CANARY_SEQ25_FIRST_VISIT_RECOVERY_DECISION_SHA256,
      decidedAt: decision.decided_at,
      functionalReleaseSequence: 25,
      launchRecords: launch.record_count,
      launchCellsPerRelay: launch.cell_count_per_relay
    })
    return
  }
  if (release.releaseSequence === 24) {
    const decision = readCanaryOwnerDecision(
      CANARY_SEQ24_DESCRIPTOR_DRIVEN_DECISION_FILE,
      CANARY_SEQ24_DESCRIPTOR_DRIVEN_DECISION_SHA256)
    const activation = decision.activation || {}
    const authority = decision.authority || {}
    const launch = activation.launch_seed || {}
    const rootCause = decision.root_cause || {}
    ...
    const followups = Array.isArray(decision.followups) ? decision.followups.join('\n') : ''
    if (decision.schema_version !== 5 ||
        decision.decision_id !== 'peerit-seq24-descriptor-driven-admission-20260802' ||
        decision.status !== 'DECIDED' ||
        !String(decision.decision || '').startsWith('ACCEPT Peerit release sequence 24 as the LIVE bounded-public-test launch successor') ||
        !followups.includes('GA product gate remains honestly blocked')) {
      throw new Error('sequence-24 descriptor-driven admission decision is not the recorded owner ACCEPT')
    }
    if (authority.baseline_release_sequence !== 23 ||
        !Array.isArray(authority.cited_prior_decisions) ||
        !authority.cited_prior_decisions.some(row => row.file === CANARY_SEQ23_FETCH_BINDING_DECISION_FILE && row.sha256 === CANARY_SEQ23_FETCH_BINDING_DECISION_SHA256)) {
      throw new Error('sequence-24 decision does not cite the seq-23 fetch-binding decision it extends')
    }
    if (!Array.isArray(rootCause.baked_pins_replaced) ||
        rootCause.baked_pins_replaced.length < 5 ||
        !rootCause.baked_pins_replaced.some(row => String(row).includes('sameLimitedAdmissionProfile')) ||
        !rootCause.baked_pins_replaced.some(row => String(row).includes('parameterHash')) ||
        !String(rootCause.no_weakening || '').includes('descriptor signature')) {
      throw new Error('sequence-24 decision does not bind the exact descriptor-driven admission fix')
    }
    if (activation.functional_release_sequence !== 24 ||
        activation.rollback_release_sequence !== null ||
        activation.rollback_posture !== 'SUPERSEDED_FOR_THIS_SLOT_BY_OWNER_DECISION_2026-08-02' ||
        activation.limited_cell_get_authority_release_sequence !== 24 ||
        activation.limited_cell_get_runtime_authority_exposed !== true ||
        activation.seed_recovery_enabled !== true ||
        activation.claim_boundary !== 'LIVE_PUBLIC_TEST_ONLY' ||
        JSON.stringify(activation.relays) !== JSON.stringify(['dal-1', 'syd-1']) ||
        ...
        activation.all_five_successor !== 'EXCLUDED' ||
        activation.ga_product_gate !== 'BLOCKED — 22 blockers DISCLOSED-OPEN, none cleared by this scope') {
      throw new Error('sequence-24 decision does not bind the exact live launch successor scope')
    }
    if (!Array.isArray(extension.files) ||
        !extension.files.includes('js/substrate/relay-consumer.js') ||
        !extension.files.includes('js/substrate/limited-cell-get-profile.mjs') ||
        !extension.files.includes('peerit-limited-cell-get-profile-v1.json') ||
        !extension.files.includes('scripts/production-pin-history-ceremony.mjs')) {
      throw new Error('sequence-24 decision does not record the ceremony extension surface')
    }
    if (bootstrap.path !== 'deploy/peerit-seed-bootstrap-v1-seq24.json' ||
        bootstrap.sha256 !== '478fc8bc537276ef14d18dbda3e5463634166c593f28a0a8a6175dbcc1946576' ||
        bootstrap.embedded_release_sequence !== 24 ||
        bootstrap.bootstrap_sequence !== 0 ||
        bootstrap.previous_bootstrap_hash !== null ||
        bootstrap.discovery_authority !== '691d524a1c2ac38de86ed592fbae6f9a906770b96fe704d3c63397a23171f6ec' ||
        bootstrap.records !== 39) {
      throw new Error('sequence-24 decision does not bind the exact launch seed bootstrap')
    }
    if (csp.policy_file !== 'deploy/render-security-headers.json' ||
        csp.policy_file_sha256 !== 'dc97c87d08bb773712cc739c37ffd4c62c121f7c92c01b20b3a8bf4a14f95724' ||
        csp.script_src !== "'self'" || csp.unsafe_eval !== 'FORBIDDEN' ||
        csp.wasm_unsafe_eval !== 'FORBIDDEN' || csp.expansion !== 'FORBIDDEN' ||
        csp.parameter_url_origin_addition !== 'FORBIDDEN') {
      throw new Error('sequence-24 decision does not bind the exact unchanged policy')
    }
    if (exactUrl.utf8 !== 'https://evidence.example:443/admission.cenc' ||
        exactUrl.utf8_hex !==
          '68747470733a2f2f65766964656e63652e6578616d706c653a3434332f61646d697373696f6e2e63656e63' ||
        ...
        exactUrl.csp_change !== 'FORBIDDEN' ||
        exactUrl.comparison !== 'EXACT_SIGNED_UTF8_BYTES') {
      throw new Error('sequence-24 decision does not bind the exact no-fetch parameterUrl contract')
    }
    const gate24 = runtimeGate.sequence_24 || {}
    if (runtimeGate.script !== 'scripts/browser-peerit-production-runtime-gate.mjs' ||
        runtimeGate.functional_mode !== 'live-two-relay' ||
        runtimeGate.functional_release_sequence !== 24 ||
        runtimeGate.full_authority_loader !== 'loadPeeritBrowserRuntimeAuthorityV1' ||
        runtimeGate.production_pin_history_loader !== 'loadPeeritProductionPinHistoryTerminalV1' ||
        runtimeGate.authority_active_before_relay_io !== true ||
        gate24.expected_network_gets !== 40 || gate24.expected_fallback_count !== 1 ||
        gate24.expected_record_count !== 39 || gate24.expected_cell_get_requests !== 40 ||
        gate24.expected_successful_cell_gets !== 39 ||
        gate24.expected_network_puts !== 0 || gate24.expected_parameter_url_requests !== 0 ||
        gate24.expected_sizeclass_1_responses !== 36 ||
        gate24.expected_sizeclass_2_responses !== 3 ||
        gate24['dal-1']?.origin !== 'https://relay-dal.p2phiverelay.xyz' ||
        gate24['dal-1']?.injected_cell_get_failures !== 1 ||
        gate24['dal-1']?.successful_cell_gets !== 38 ||
        gate24['dal-1']?.verified_readback_evidence_count !== 38 ||
        gate24['syd-1']?.origin !== 'https://relay-syd.p2phiverelay.xyz' ||
        gate24['syd-1']?.injected_cell_get_failures !== 0 ||
        gate24['syd-1']?.successful_cell_gets !== 1 ||
        gate24['syd-1']?.verified_readback_evidence_count !== 1) {
      throw new Error('sequence-24 decision does not bind the named-relay production runtime gate')
    }
    if (relays.admission_semantics == null ||
        !String(relays.admission_semantics).startsWith('descriptor-driven') ||
        relays['dal-1']?.minimum_descriptor_sequence !== 13 ||
        relays['dal-1']?.descriptor_head_sha256 !== '34541f0f9364a22259479640254e26cef8cab30e89208ba22f6e3cdee5633ad4' ||
        relays['syd-1']?.minimum_descriptor_sequence !== 16 ||
        relays['syd-1']?.descriptor_head_sha256 !== '71bd00aeb6419c859b99fb37f41e834a1d5bf9232568dad2bb1344a1dac9f613' ||
        relays['dal-1']?.admission_protocol_sha256 != null ||
        relays['syd-1']?.admission_protocol_sha256 != null) {
      throw new Error('sequence-24 decision does not bind the descriptor-driven admission authority (descriptor pins present, admission hash deliberately absent)')
    }
    addCheck('canary:owner-decision', 'pass', `Owner descriptor-driven admission decision verified byte-exact (sha256 ${CANARY_SEQ24_DESCRIPTOR_DRIVEN_DECISION_SHA256.slice(0, 12)}...): ACCEPT seq-24 as the LIVE bounded-public-test launch successor — admission parameterHash expected from the CURRENT signature-verified descriptor everywhere (no baked pin anywhere), limited Cell-GET exposed, 39-record signed recovery enabled; unchanged CSP, zero PUTs, all-five excluded, GA gate still blocked with 22 blockers DISCLOSED-OPEN.`, {
      file: CANARY_SEQ24_DESCRIPTOR_DRIVEN_DECISION_FILE,
      sha256: CANARY_SEQ24_DESCRIPTOR_DRIVEN_DECISION_SHA256,
      decidedAt: decision.decided_at,
      functionalReleaseSequence: 24,
      launchRecords: launch.record_count,
      launchCellsPerRelay: launch.cell_count_per_relay
    })
    return
  }
  if (release.releaseSequence === 23) {
    const decision = readCanaryOwnerDecision(
      CANARY_SEQ23_FETCH_BINDING_DECISION_FILE,
      CANARY_SEQ23_FETCH_BINDING_DECISION_SHA256)
    const activation = decision.activation || {}
    const authority = decision.authority || {}
    const launch = activation.launch_seed || {}
    const rootCause = decision.root_cause || {}
    ...
    const followups = Array.isArray(decision.followups) ? decision.followups.join('\n') : ''
    if (decision.schema_version !== 5 ||
        decision.decision_id !== 'peerit-seq23-fetch-binding-recovery-fix-20260801' ||
        decision.status !== 'DECIDED' ||
        !String(decision.decision || '').startsWith('ACCEPT Peerit release sequence 23 as the LIVE bounded-public-test launch successor') ||
        !followups.includes('GA product gate remains honestly blocked')) {
      throw new Error('sequence-23 fetch-binding decision is not the recorded owner ACCEPT')
    }
    if (authority.baseline_release_sequence !== 22 ||
        !Array.isArray(authority.cited_prior_decisions) ||
        !authority.cited_prior_decisions.some(row => row.file === CANARY_SEQ22_COMPRESSION_TOLERANCE_DECISION_FILE && row.sha256 === CANARY_SEQ22_COMPRESSION_TOLERANCE_DECISION_SHA256)) {
      throw new Error('sequence-23 decision does not cite the seq-22 compression-tolerance decision it extends')
    }
    if (!String(rootCause.mechanism || '').includes('UNBOUND globalThis.fetch') ||
        !Array.isArray(rootCause.fix) ||
        !rootCause.fix.some(row => String(row).includes('relay-consumer.js')) ||
        !rootCause.fix.some(row => String(row).includes('codex/blind-client-fetch-binding-20260801'))) {
      throw new Error('sequence-23 decision does not bind the exact fetch-binding root cause and fix')
    }
    if (activation.functional_release_sequence !== 23 ||
        activation.rollback_release_sequence !== null ||
        activation.rollback_posture !== 'SUPERSEDED_FOR_THIS_SLOT_BY_OWNER_DECISION_2026-08-01' ||
        activation.limited_cell_get_authority_release_sequence !== 23 ||
        activation.limited_cell_get_runtime_authority_exposed !== true ||
        activation.seed_recovery_enabled !== true ||
        activation.claim_boundary !== 'LIVE_PUBLIC_TEST_ONLY' ||
        JSON.stringify(activation.relays) !== JSON.stringify(['dal-1', 'syd-1']) ||
        ...
        activation.all_five_successor !== 'EXCLUDED' ||
        activation.ga_product_gate !== 'BLOCKED — 22 blockers DISCLOSED-OPEN, none cleared by this scope') {
      throw new Error('sequence-23 decision does not bind the exact live launch successor scope')
    }
    if (!Array.isArray(extension.files) ||
        !extension.files.includes('js/substrate/relay-consumer.js') ||
        !extension.files.includes('scripts/production-pin-history-ceremony.mjs') ||
        ...
        !extension.files.includes('peerit-limited-cell-get-profile-v1.json')) {
      throw new Error('sequence-23 decision does not record the ceremony extension surface')
    }
    if (bootstrap.path !== 'deploy/peerit-seed-bootstrap-v1-seq23.json' ||
        bootstrap.sha256 !== 'b0d9b996e112fb542b5577a7fad179ccad6333eb624b256168118c43e3176bec' ||
        bootstrap.embedded_release_sequence !== 23 ||
        bootstrap.bootstrap_sequence !== 0 ||
        bootstrap.previous_bootstrap_hash !== null ||
        bootstrap.discovery_authority !== '691d524a1c2ac38de86ed592fbae6f9a906770b96fe704d3c63397a23171f6ec' ||
        bootstrap.records !== 39) {
      throw new Error('sequence-23 decision does not bind the exact launch seed bootstrap')
    }
    if (csp.policy_file !== 'deploy/render-security-headers.json' ||
        csp.policy_file_sha256 !== 'dc97c87d08bb773712cc739c37ffd4c62c121f7c92c01b20b3a8bf4a14f95724' ||
        csp.script_src !== "'self'" || csp.unsafe_eval !== 'FORBIDDEN' ||
        csp.wasm_unsafe_eval !== 'FORBIDDEN' || csp.expansion !== 'FORBIDDEN' ||
        csp.parameter_url_origin_addition !== 'FORBIDDEN') {
      throw new Error('sequence-23 decision does not bind the exact unchanged policy')
    }
    if (exactUrl.utf8 !== 'https://evidence.example:443/admission.cenc' ||
        exactUrl.utf8_hex !==
          '68747470733a2f2f65766964656e63652e6578616d706c653a3434332f61646d697373696f6e2e63656e63' ||
        ...
        exactUrl.csp_change !== 'FORBIDDEN' ||
        exactUrl.comparison !== 'EXACT_SIGNED_UTF8_BYTES') {
      throw new Error('sequence-23 decision does not bind the exact no-fetch parameterUrl contract')
    }
    const gate23 = runtimeGate.sequence_23 || {}
    if (runtimeGate.script !== 'scripts/browser-peerit-production-runtime-gate.mjs' ||
        runtimeGate.functional_mode !== 'live-two-relay' ||
        runtimeGate.functional_release_sequence !== 23 ||
        runtimeGate.full_authority_loader !== 'loadPeeritBrowserRuntimeAuthorityV1' ||
        runtimeGate.production_pin_history_loader !== 'loadPeeritProductionPinHistoryTerminalV1' ||
        runtimeGate.authority_active_before_relay_io !== true ||
        gate23.expected_network_gets !== 40 || gate23.expected_fallback_count !== 1 ||
        gate23.expected_record_count !== 39 || gate23.expected_cell_get_requests !== 40 ||
        gate23.expected_successful_cell_gets !== 39 ||
        gate23.expected_network_puts !== 0 || gate23.expected_parameter_url_requests !== 0 ||
        gate23.expected_sizeclass_1_responses !== 36 ||
        gate23.expected_sizeclass_2_responses !== 3 ||
        gate23['dal-1']?.origin !== 'https://relay-dal.p2phiverelay.xyz' ||
        gate23['dal-1']?.injected_cell_get_failures !== 1 ||
        gate23['dal-1']?.successful_cell_gets !== 38 ||
        gate23['dal-1']?.verified_readback_evidence_count !== 38 ||
        gate23['syd-1']?.origin !== 'https://relay-syd.p2phiverelay.xyz' ||
        gate23['syd-1']?.injected_cell_get_failures !== 0 ||
        gate23['syd-1']?.successful_cell_gets !== 1 ||
        gate23['syd-1']?.verified_readback_evidence_count !== 1) {
      throw new Error('sequence-23 decision does not bind the named-relay production runtime gate')
    }
    if (relays['dal-1']?.minimum_descriptor_sequence !== 12 ||
        relays['dal-1']?.descriptor_head_sha256 !== 'badfc830eb4eae7e25573e73873b6420801aafb00eab4250a61160e720399440' ||
        relays['dal-1']?.admission_protocol_sha256 !== '7d29e79d20955f14b4553d8517c35a5b548308ff9f44db39590c0ec5c2768ecb' ||
        relays['syd-1']?.minimum_descriptor_sequence !== 15 ||
        relays['syd-1']?.descriptor_head_sha256 !== '641e01b9777922d6cab36b4bf7411f762e45f8c53c9416006eb00ff352c809c2' ||
        relays['syd-1']?.admission_protocol_sha256 !== '108e53b1e93c56e8c37f281234c23035b888615e8cd904d2751aa58b2dcea120') {
      throw new Error('sequence-23 decision does not bind the same-day named relay authorities')
    }
    addCheck('canary:owner-decision', 'pass', `Owner fetch-binding root-cause decision verified byte-exact (sha256 ${CANARY_SEQ23_FETCH_BINDING_DECISION_SHA256.slice(0, 12)}...): ACCEPT seq-23 as the LIVE bounded-public-test launch successor — bind the fallback fetch in the recovery path and the re-vendored blind clients (root-caused Illegal invocation -> 'descriptor bootstrap failed'), limited Cell-GET exposed, 39-record signed recovery enabled; unchanged CSP, zero PUTs, all-five excluded, GA gate still blocked with 22 blockers DISCLOSED-OPEN.`, {
      file: CANARY_SEQ23_FETCH_BINDING_DECISION_FILE,
      sha256: CANARY_SEQ23_FETCH_BINDING_DECISION_SHA256,
      decidedAt: decision.decided_at,
      functionalReleaseSequence: 23,
      launchRecords: launch.record_count,
      launchCellsPerRelay: launch.cell_count_per_relay
    })
    return
