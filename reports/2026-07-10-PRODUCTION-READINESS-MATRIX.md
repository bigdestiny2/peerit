...
| 2,000-client release capacity | **Fail — hard GA gate** | `npm run launch:readiness` now rejects the former local 30-client report and requires a passing, explicitly staging-labelled 2,000-client atomic evidence report; local 200-client atomic run had correctness success but p99 4,866ms | Run isolated staging sweep at 100/500/1,000/2,000 with shared-NAT and distributed profiles, relay failure, and reviewed RSS budgets. The report label is operator-attested; a local co-located run is diagnostic only. |
