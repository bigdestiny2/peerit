...
| BlindShard key splitting and split-transport designs | Tested replicating a complete ciphertext while threshold-splitting its decryption key, and designed separate ingress and storage roles. This can limit what one non-colluding operator can decrypt; it does not hide ciphertext or metadata, and depends on independent operators plus tested repair and recovery. | **Experimental/design work**, disabled and not shipped |
| Browser-native Hypercore/Corestore on IndexedDB | An older Hypercore 10/Corestore 6 prototype proved basic random-access IndexedDB reopen, truncate and isolation behavior. Porting the current Hypercore 11/Corestore 7 stack still needs real crash, quota, snapshot, iterator, multi-tab and transport evidence. | **Old-stack prototype; current-stack design unproven**, deliberately not a release claim |
