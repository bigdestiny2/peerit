...
async function main () {
  ...
  const live = new BridgeGossipSync({ pear: world, getMe: () => readerId.me().pubkey, identity: readerId, storage: readerStore, validate: makeValidator(BITS), pollMs: 0, seedOutboxes })
  await live.ready()
  await live._refresh()
  ...
  const t0 = Date.now()
  const offline = new BridgeGossipSync({ pear: deadPear(), getMe: () => readerId.me().pubkey, identity: readerId, storage: readerStore, validate: makeValidator(BITS), pollMs: 0, seedOutboxes, instantBoot: true })
  await offline.ready()
  ...
  const late = new BridgeGossipSync({ pear: lazy.pear, getMe: () => readerId.me().pubkey, identity: readerId, storage: lateStore, validate: makeValidator(BITS), pollMs: 0, seedOutboxes, instantBoot: true })
  await late.ready()
  ...
  const fresh = new BridgeGossipSync({ pear: deadPear(), getMe: () => freshId.me().pubkey, identity: freshId, storage: mem(), validate: makeValidator(BITS), pollMs: 0, instantBoot: true, seedSnapshot: snapshot })
  await fresh.ready()
  ...
  const fresh2 = new BridgeGossipSync({ pear: deadPear(), getMe: () => freshId.me().pubkey, identity: freshId, storage: mem(), validate: makeValidator(BITS), pollMs: 0, instantBoot: true, seedSnapshot: tampered })
  await fresh2.ready()
  ...
  const classic = new BridgeGossipSync({ pear: world, getMe: () => readerId.me().pubkey, identity: readerId, storage: mem(), validate: makeValidator(BITS), pollMs: 0, seedOutboxes })
  await classic.ready()
