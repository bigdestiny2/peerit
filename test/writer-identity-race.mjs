...
console.log('— public-web UI and recovery gates —')
{
  ...
  assert.match(source, /withWriterSession: requiresSerializedWebWriter\(\) \? withDataWriterSession : null/)
  assert.match(source, /return isPeeritSubstrateRuntime\(\)[\s\S]*withLocalDataWriterSession\(fn\)[\s\S]*withAtomicDataWriterSession\(fn\)/)
  assert.match(source, /sync\.withLocalWriterSession/)
