...
function publicationStatus (summary, relays, relayHints, localState = 'ready', localWarning = null, localFailure = null) {
  ...
  else if (acknowledgedTargets > 0) relayState = 'relay-acknowledged'

  const policyDurable = !!(latest && latest.policyDurable)
  const readbackVerified = latest ? latest.readbackVerified || 0 : 0
  let durabilityState = 'local-only'
  if (policyDurable) durabilityState = 'policy-durable'
  else if (readbackVerified > 0) durabilityState = 'recently-retrievable'
  else if (acknowledgedTargets > 0) durabilityState = 'remote-single'
  ...
  return Object.freeze({
    ...
    durability: Object.freeze({
      ...
      acknowledgedReplicas: acknowledgedTargets,
      readbackVerified
    }),
...
export class PeeritSubstrateSync {
  ...
  async status () {
    try { this._adoptSummary(await this.journal.summary()) } catch (error) { this._localFailure = error }
  ...
  async flushPublicationQueue () {
    ...
    const run = (async () => {
      ...
      if ((summary.targetStateCounts.preparing || 0) > 0 || (summary.targetStateCounts.delivering || 0) > 0) {
        const recovered = await this.journal.recoverExpiredClaims(this.clock())
        if (recovered) await this._refreshAfterMutation(false, [])
      }
      ...
      const retries = typeof this.journal.listRetryIntentIds === 'function'
        ...
        : { intentIds: [] }
      const intentIds = [...new Set([...page.intentIds, ...retries.intentIds])]
      await runBounded(intentIds, this.deliveryIntentConcurrency, intentId => this._deliverIntent(intentId, activeRelays))
  ...
  async _deliverIntent (intentId, activeRelays = this._activeRelays()) {
    await runBounded(activeRelays, this.deliveryRelayConcurrency, relay => this._deliverIntentToRelay(intentId, relay))
  }
  ...
  async _deliverIntentToRelay (intentId, relay) {
    ...
    try {
      ...
      if (completed) await this._refreshAfterMutation(false, [])
    } catch (error) {
  ...
  _scheduleNextAttempt () {
    ...
    this.journal.nextWake({
      ...
      if (delay != null) this._scheduleFlush(delay)
    }).catch(error => {
      this._localFailure = error
      console.error(error)
    })
