...
// call, and the pool merges them across relays taking the highest VERIFIED
// version. A client bootstraps its rollback floor from it at boot — so even a
...
async function main () {
  ...
  ok(floorOf(carol.storage)[alice.pub] && floorOf(carol.storage)[alice.pub].v === 3, 'carol bootstraps v3 (the pool took the max across relays; A\'s stale v1 directory did not fool her)')

  console.log('\n— the bootstrapped floor arms rollback detection from the first read —')
  storeOf(world, B, alice.pub).rows.set(keys.head(alice.pub), v1Head) // now BOTH relays roll back to v1
