...
- **THE BLIND BLOB SURFACE LANDED (2026-07-02, HiveRelay PR #149 `feat/shard-store`, another agent):** `shard:<hash>` content-addressed store — `POST/GET/HEAD/DELETE /api/v1/shard`, addressing = **`blake2b-256(ciphertext)`** (`sodium.crypto_generichash`), PUT auth = custody-pin (default) or payment, self-verifying GET, possession proofs, byzantine k-of-n recover. Manifest binds `shareIndex → shard:<hash>` inside the signed custody-intent (`commitmentRoot=merkleRoot(sorted shareCommitments)`). Spec: hiverelay `docs/BLIND-SHARD-STORE-SPEC.md`. Still UNMERGED + Phase-3-live still needs ≥3 INDEPENDENT relays.
- **peerit STORE-READY (blake2b reconciliation done):** `blob-disperse` shard content-address is now an **injectable `hashShard`** (default SHA-256; the real `/api/v1/shard` adapter injects blake2b-256 from the DHT bundle's sodium) — the module is hash-agnostic, proven by `test/blob-disperse.mjs` (round-trips through an injected distinct hash; wrong read-hash fails the gate). `shard.js` untouched (its SHA-256 `.id` is ignored; `decode` needs only `{index,bytes}`). blobId stays SHA-256 (peerit-internal ciphertext gate, NOT a store address). REMAINING at merge: the concrete `/api/v1/shard` backend adapter (`putShard`/`getShard` + custody-pin signing + shareManifest mapping) — build against the MERGED store, not the PR.
- **CLIENT GLUE BUILT (2026-07-02), unwired + not in SITE_FILES:** `js/blob-disperse.js` — `disperseBody` (box → RS-encode → `place()` → `putShard` per replica → signed manifest `{blobId,contentKey,iv,k,n,replicas,shardIds}`) + `reassembleBody` (recompute placement → `getShard` → content-address check → RS-decode → `SHA-256(C)==blobId` → unbox). **Dependency-injected** against the handover interface (`{putShard,getShard}`, [`BLINDSHARD-BLOB-SURFACE-HANDOVER.md` §4](BLINDSHARD-BLOB-SURFACE-HANDOVER.md)), validated against a FAKE fleet (`test/blob-disperse.mjs`, 13 checks: round-trip, K-of-N tolerance, tamper-route, `<K`-per-relay). This is the CLIENT LOGIC proven against a fake — **NOT** live dispersal. To activate: land the real `shard:<hash>` blob surface + a concrete backend adapter (over `dht-relay-ws`), reach **≥3 independent relays**, then branch `data.js` `_boxBody`/`_hydrate` on `shouldDisperse`. Mirrors the dht-adapter "logic first, live wire later" pattern.
