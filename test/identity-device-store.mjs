...
async function main () {
  ...
  const emptyRaceKv = memoryKv()
  const emptyRaceStore = createIdentityStore({ kv: emptyRaceKv })
  ok((await emptyRaceStore.inspect()).status === 'empty', 'import can explicitly observe a genuinely empty device tier')
  await emptyRaceKv.putIfAbsent('identity:v1', { v: 1, key: { broken: true }, iv: { malformed: true }, ct: { malformed: true }, pubkey: null })
  await assert.rejects(() => emptyRaceStore.replace(entry, { expectedEmpty: true }), /changed in another tab/, 'exact-empty CAS refuses a corrupt null-pubkey generation that appeared after empty inspection')
  passed++; console.log('  ✓ exact-empty CAS refuses a corrupt null-pubkey generation that appeared after empty inspection')
  ok((await emptyRaceStore.inspect()).status === 'corrupt', 'failed exact-empty import leaves the raced corrupt generation intact for explicit recovery')
  const nullValueKv = memoryKv()
  await nullValueKv.putIfAbsent('identity:v1', null)
  const nullValueStore = createIdentityStore({ kv: nullValueKv })
  await assert.rejects(() => nullValueStore.replace(entry, { expectedEmpty: true }), /changed in another tab/, 'exact-empty CAS checks key absence, not a null-like stored value')
  passed++; console.log('  ✓ exact-empty CAS checks key absence, not a null-like stored value')
