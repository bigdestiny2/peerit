...
async function main () {
  ...
  harness.context.fetch = async (path) => new Response(nextBodies[String(path)], { status: 200 })
  ...
  assert.deepEqual([...harness.cacheMap.get(nextCache).entries], Object.entries(nextBodies), 'successful candidate caches every verified asset')
  assert.equal(harness.skipWaitingCalls(), 1, 'complete verified candidate requests activation once')

  console.log('service-worker-atomic: passed 12 checks')
}
