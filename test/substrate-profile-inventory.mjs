...
test('canonical profile and local publication slices clear only proven blockers and remain non-release-ready', () => {
  ...
  assert.equal(PEERIT_PROFILE_STATUS.validatorArtifact.contextualWebAssetManifestGraphReady, true)
  assert.equal(PEERIT_PROFILE_STATUS.validatorArtifact.contextualGraphRuntimeUnavailableSchemaCount, 41)
  assert.equal(PEERIT_PROFILE_STATUS.validatorArtifact.signedValidatorBundleGraphAuthorityFactoryReady, false)
  assert.equal(PEERIT_PROFILE_STATUS.validatorArtifact.browserContextualGraphHarnessComplete, false)
